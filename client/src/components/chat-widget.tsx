    ...
    onSuccess: (data, variables) => {
      ...
      if (selectedChat && selectedChat.id === variables.chatRoomId) {
        setSelectedChat(prev => {
          if (!prev) return null;
          // Remove temp message with same content and senderId
          const filteredMessages = prev.messages.filter(msg =>
            !(msg._id && msg._id.startsWith('temp-') && msg.content === realMessage.content && msg.senderId === realMessage.senderId)
          );
          return { ...prev, messages: [...filteredMessages, realMessage] };
        });
      }
    },
    onError: (error) => {
